...
def br_translate(lang, text):
    prompt = f"""
Human: Translate the following text to {lang} language. 
If the source language is not specified, assume it is English.
Do not include any other preceding text.
    ...
    body=json.dumps(
        {
            ...
            "max_tokens": 1000,
            "system": "Please respond only with JSON formatted data",
            "temperature": 0.2,
            "messages": prompt
        }  
    )
